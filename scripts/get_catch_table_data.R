...
get_catch_table_data <- function(ship, survey, data_set_id, analysis_id, scaling_hauls_only = TRUE){
  ...
  # return a table for each region in the requested survey 
  
  get_catch_table_for_region <- function(region){
  
    # use this to get a list of parameters for the requested region and gear type
    hauls_list <- event_data$EVENT_ID[event_data$region == region]
    report_num <- unique(event_data$REPORT_NUMBER[event_data$region == region])
    ...
    if (scaling_hauls_only == TRUE) {
      ...
      # identify the methot hauls (if there are any)
      methots_in_region <- event_data$EVENT_ID[event_data$region == region & event_data$`Gear type` == "Methot"]
  
      hauls_list <- hauls_list[hauls_list %in% hauls_in_nh$EVENT_ID | hauls_list %in% methots_in_region]
    }
    ...
    catch_data$REPORT_NUMBER <- report_num
    catch_data$region <- region
    
    return(catch_data)
  
  }
  
  # get the catch data for each region in the survey
  survey_catch_data <- purrr::map_dfr(unique(event_data$region), get_catch_table_for_region)

  # return dataframe
  return(survey_catch_data)
}
