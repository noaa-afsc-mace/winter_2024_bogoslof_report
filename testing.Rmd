...
---
  ...
  <!-- This document is currently written to produce a 'first draft' of the winter Shelikof/Shumagins cruise report-->
...
data_set_path <- "G:/DY2403/cruise report/testing/"


# set your current survey parameters for the SHELIKOF survey. 
current_shelikof_survey <- TRUE # set to FALSE if no survey!
# IF no survey- put 0 for parameters!
ship_shelikof <- 157 
survey_shelikof <- 202304
data_set_shelikof <- 1
analysis_shelikof <- 1
zones_shelikof <- c(0)
# IF you don't have a bottom referenced survey or dataset, enter 0 for those.
bot_ref_data_set_shelikof <- 2
bot_ref_analysis_shelikof <- 1
# set the comparison datasets for the shelikof (if none, put 0)
comp_data_sets_shelikof <- c(1)
comp_analyses_shelikof <- c(2)
# Identify the Shelikof Strait report number
# (see macebase2.report_definitions to check for the right number)
report_number_shelikof <- 1

# set your current survey parameters for the SHUMAGINS survey. 
current_shumagins_survey <- TRUE # set to FALSE if no survey!
# IF no survey- put 0 for parameters!
ship_shumagins <- 157
survey_shumagins <- 202303
data_set_shumagins <- 1
analysis_shumagins <- 1
zones_shumagins <- c(0)
# IF you don't bottom referenced survey or dataset, enter 0 for those.
bot_ref_data_set_shumagins <- 2
bot_ref_analysis_shumagins <- 1
# set the comparison datasets for the shumagins (if none, put )
comp_data_sets_shumagins <- c(1)
comp_analyses_shumagins <- c(2)

# IF any other surveys happened, identify as above!
...
primary_sensor <- "TSG-SBE38-Temp_1_min_avg"
backup_sensor <- "Mid-SeaTemp-C-Cal-VALUE_1_min_avg"
...
# shelikof
eva_shelikof_perc <- 4.6
eva_chirikof_perc <- 13.1
eva_marmot_perc <- 8.0

# shumagins
eva_shumagins_perc <- 8.7
eva_sanak_perc <- NA
eva_morzhovoi_perc <- 5.4
eva_pavlof_perc <- 13.6
...
# shumagins
rotation_shumagins <- 5
max_bar_height_shumagins <- 2

# shelikof
rotation_shelikof <- 1
max_bar_height_shelikof <- 0.5
...
goa_labels_to_get <- c(
  ...
  "Kodiak Island", "Cape Kekurnoi", "Cape Nukshak",
  "Kalsin Bay", "Cape Igvak", "Morzhovoi Bay", "Pavlof Bay", "Sanak Island",
  "Marmot Bay", "Chirikof Shelfbreak")

# of this list, should any of these be indicated with an arrow? I.e. are they in a really crowded area?
goa_arrow_list <- c("Shumagin Islands", "Shelikof Strait",
  "Cape Kekurnoi", "Cape Nukshak",
  "Kalsin Bay", "Cape Igvak", "Morzhovoi Bay", "Pavlof Bay", 
  "Marmot Bay")

# Specifying one or multiple SBE profiles to be plotted
SBE_Nset <- 1
# only configured for plotting 1 or 2 profiles at this time based on class (i.e. SS1 or SS2 hauls)

# Specify hauls by event number for each profile that you wish to plot
if (SBE_Nset > 1) {
  SBE_set.ls <- list(
    "haul_set1" = c(1:12, 14, 16, 18, 19), # specify event numbers for subset 1 of hauls you wish to plot
    "haul_set2" = c(13, 15, 17, 21)
  ) # specify event numbers for subset 2 of hauls you wish to plot
} else {
  SBE_set.ls <- NULL
}
...
survey_params_shelikof_selectivity_corrected_surveys <- "historical_data/parameters_shelikof_length_age_time_series_selectivity_corrected.csv"
survey_params_shumagins_selectivity_corrected_surveys <- "historical_data/parameters_shumagins_length_age_time_series_selectivity_corrected.csv"

# historical bottom-and surface-referenced dataset info
# TODO:: use analysis_tags table when it is ready!
shelikof_surf_and_bot_historical_path <- "historical_data/shelikof_bot_and_surf_ref_historical.csv"
shumagins_surf_and_bot_historical_path <- "historical_data/shumagins_bot_and_surf_ref_historical.csv"

# data:
# historical biomass and numbers at length figure- these are used for Shelikof Stait + Shumagin Island 
# nums/biomass at length figures, so only contain those two regions
# lengths updated by ALM 9/14/23 to be just the pre-selectivity values
pre_selectivity_biomass_nums_path <- 'historical_data/historical_biomass_nums.rds'

# Shelikof Strait only historical ages- unlike the lengths, these include every year in the time series
pre_selectivity_biomass_nums_ages_path <- 'historical_data/historical_biomass_nums_ages.rds'

# historical trawl weights and GSI for abundance-weighted maturity
historical_trawl_weights_path <- "historical_data/historical_trawl_weights.rds"
historical_GSI_path <- "historical_data/historical_gsi.rds"
...
eva_path <- "historical_data/historical_eva.rds"
...
# adding in the shelikof boundaries for the COG plot
shelikof_basemap <- st_read("shapefiles/winterShelikof_95pcent_domain_1995-21/spPoly.05_edited_v5.shp")
shelikof_basemap <- st_transform(shelikof_basemap, crs = 3338)
...
# this will let us grab info that's been run in a chunk for the figure captions
...
current_year <- as.numeric(substr(survey_shelikof, 1, 4))
...
if (update_datasets == TRUE) {
  ...
  load(file = paste0(data_set_path, "winter_", current_year,  "_dataset.RData"))
...
# APPENDIX III. REGIONAL SUMMARIES

<!-- Area summaries: for every region other than Shelikof, create a summary with tables, figures, text -->
```{r region_appendix, echo = FALSE, message=FALSE, warning=FALSE, results = 'asis'}

# we'll print these in date order from when the survey was started in a given region
# we'll also ignore shelikof here, since it's already been reported

# collect the regions to report and order by the start time
report_regions_list <- current_year_interval_data %>%
  st_drop_geometry() %>%
  filter(region != "Shelikof Strait") %>%
  group_by(SHIP, SURVEY, region) %>%
  summarize(start_time = min(START_TIME)) %>%
  arrange(start_time)

for (i in report_regions_list$region){
  tmp_doc <- knitr::knit_child("appendix_region_summary.Rmd", quiet = TRUE)
  cat(tmp_doc)
}
